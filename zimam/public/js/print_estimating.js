...
// (تُختبر بالحالات نفسها عبر tools/test_print_estimating_js.mjs). تعمل في المتصفح وفي Node.
(function (root, factory) {
	...
	const SIZE_PRESETS = { A6: [105, 148], A5: [148, 210], A4: [210, 297], A3: [297, 420], B5: [176, 250], "بطاقة أعمال 90×55": [90, 55], "DL 99×210": [99, 210], "20×20": [200, 200] };
	...
	function componentCost(comp, qty, paper, machine, wastePercent, bleed) {
		...
		const [rw, rh, pieces] = runSheet(paper, machine);
		const [ups, orientation] = imposition(pw, ph, rw, rh, bleed, flt(machine.gripper_margin_mm));
	...
	function finishingAmount(row, service, q) {
		...
		return r3(Math.max(q * rate + setup, flt(service.min_charge)));
	}
	...
	function priceFromCost(totalCost, settings, doc) {
		const margin = flt(doc.margin_percent, flt(settings.default_margin_percent));
		...
		return [r3(price), r3(rushAmount), r3(discount)];
	}
	...
	function compute(doc, masters, settings) {
		...
		function run(q) {
			if (q <= 0) {
				return {
					...
					finishing: (doc.finishing || []).map((r) => ({ basis: r.basis || BASIS_PER_COPY, qty: 0.0, rate: 0.0, setup_cost: 0.0, amount: 0.0 })),
					extras: (doc.extras || []).map(() => ({ amount: 0.0 })),
					paper_cost: 0.0, print_cost: 0.0, finishing_cost: 0.0, extras_cost: 0.0, design_cost: 0.0, total_cost: 0.0, rush_amount: 0.0, discount_amount: 0.0, net_total: 0.0, unit_price: 0.0, unit_cost: 0.0, tax_rate: taxRateOf(), tax_amount: 0.0, grand_total: 0.0, total_sheets: 0, total_impressions: 0, production_hours: 0.0,
				};
			}
			...
			for (const r of doc.finishing || []) {
				...
				finOut.push({ basis: rr.basis, qty: r3(fq), rate: r3(rate), setup_cost: r3(setup), amount: amt });
				finCost += amt;
			}
			...
			const [netTotal, rushAmount, discountAmount] = priceFromCost(totalCost, settings, doc);
			...
			return {
				...
				rush_amount: rushAmount, discount_amount: discountAmount, net_total: netTotal,
				...
				total_sheets: sheets, total_impressions: impressions, production_hours: Math.round(hours * 100) / 100,
			};
		}
		...
		result.tiers = (doc.tiers || []).map((t) => {
			const tq = cint(t.qty);
			if (tq <= 0) return { qty: tq, total_cost: 0, price: 0, unit_price: 0, grand_total: 0 };
			const r = run(tq);
			return { qty: tq, total_cost: r.total_cost, price: r.net_total, unit_price: r.unit_price, grand_total: r.grand_total };
		});
		result.estimated_days = Math.max(1, Math.ceil(result.production_hours / 8.0)) + (cint(doc.rush) ? 0 : 1);
		return result;
	}
	...
	return { compute, imposition, componentCost, spineMm, parseTiers, SIZE_PRESETS, SINGLE, COVER, INNER, INSERT, flt, cint, r3 };
});
