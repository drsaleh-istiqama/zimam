...
"""سير اعتماد سند الصرف (النواة، كما في وقاف): المحاسب يسجّل ويحيل ← مدير المالية يراجع ← معتمد الصرف (مدير المؤسسة/الرئيس التنفيذي) يعتمد (فوق الحد)
...
def ensure_payment_voucher_workflow():
	ensure_states()
	allow_self = 1 if frappe.db.get_single_value("Zimam Settings", "allow_self_approval") else 0
	...
	wf.send_email_alert = 1 if frappe.db.get_single_value("Zimam Settings", "notify_on_stage_change") else 0
