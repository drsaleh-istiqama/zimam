...
"""أرصدة الصناديق (النواة — بطاقات الصناديق في وقاف): المرحَّل (الأستاذ) · الالتزامات المعتمدة غير المدفوعة · الدفتري · الفعلي حسب الكشف · الفرق."""
import frappe
from frappe import _
from frappe.utils import flt

from zimam.zimam_core.finance import fund_balances
...
def execute(filters=None):
	...
	for fund in frappe.get_all("Treasury Fund", filters=f, fields=["name", "fund_name", "fund_type", "account", "statement_balance", "statement_date", "purpose"], order_by="fund_type, name"):
	...
	columns = [
		{"label": _("الصندوق"), "fieldname": "fund", "fieldtype": "Link", "options": "Treasury Fund", "width": 130},
