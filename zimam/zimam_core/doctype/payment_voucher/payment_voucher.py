...
"""سند صرف (النواة — لكل مؤسسة) بسلسلة اعتماد (Workflow «سند صرف — سلسلة الاعتماد»): المحاسب يسجّل ← مدير المالية ← معتمد الصرف
...
from zimam.utils import get_settings, parent_company
from zimam.zimam_core.finance import fund_account, make_journal_entry, update_project_spent
...
class PaymentVoucher(Document):
	def validate(self):
		...
		if not self.cost_center:
			self.cost_center = frappe.db.get_value("Expense Classification", self.expense_classification, "cost_center") or get_settings().default_expense_cost_center
		threshold = flt(get_settings().ceo_approval_threshold)
...
def get_defaults():
	s = get_settings()
	return {"company": s.parent_company, "fund": s.default_fund, "threshold": flt(s.ceo_approval_threshold)}
